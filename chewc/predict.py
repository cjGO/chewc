...
# %% auto 0
__all__ = ['PredictionResults', 'gblup_predict_improved', 'gblup_predict']
...
@jax.jit
def gblup_predict_improved(
    pop: Population,
    h2: float = 0.5,
    trait_idx: int = 0
) -> PredictionResults:
    """
    Perform GBLUP prediction using a single, unified Mixed Model Equation solve
    for all individuals. This approach is more efficient and JAX-friendly.
    ...
    # --- Step 1: Identify training individuals and create a masked phenotype vector ---
    train_mask = ~jnp.isnan(pheno.flatten())
    y_train = pheno[train_mask]

    # --- Step 2: Calculate the full G-matrix and its inverse ---
    G = _calculate_genomic_relationship_matrix(dosage)
    G_inv = jnp.linalg.inv(G)

    # --- Step 3: Set up the Mixed Model Equations for all individuals ---
    # The incidence matrix for fixed effects (X) is a column of ones for the intercept.
    X = jnp.ones((n_ind, 1))
    # The incidence matrix for random effects (Z) is an identity matrix.
    Z = jnp.identity(n_ind)

    # We only use the phenotyped individuals to build the right-hand side of the MME.
    ...
    # Build the left-hand side (LHS) of the MME
    XTX = X_train.T @ X_train
    XTZ = X_train.T @ Z
    ZTX = Z.T @ X_train
    # Constructing the Z'Z matrix using only the training data
    ZTZ_train = jnp.diag(train_mask.astype(jnp.float32))
    ZTZ_plus_alpha_Ginv = ZTZ_train + alpha * G_inv

    lhs_top = jnp.hstack([XTX, XTZ])
    lhs_bottom = jnp.hstack([ZTX, ZTZ_plus_alpha_Ginv])
    lhs = jnp.vstack([lhs_top, lhs_bottom])

    # Build the right-hand side (RHS) of the MME
    XTy = X_train.T @ y_train
    ZTy = Z_train.T @ y_train
    # The Z'y vector needs to be the same size as the number of individuals
    rhs_z = jnp.zeros(n_ind).at[train_mask].set(y_train.flatten())
    rhs = jnp.concatenate([XTy.flatten(), rhs_z])

    # --- Step 4: Solve the MME ---
    solutions = jnp.linalg.solve(lhs, rhs)
    n_fixed = X.shape[1]
    fixed_effects = solutions[:n_fixed]
    all_ebv = solutions[n_fixed:]

    # --- Step 5: Calculate PEV and Reliability ---
    C_inv = jnp.linalg.inv(lhs)
    C22_inv = C_inv[n_fixed:, n_fixed:]
    var_p = jnp.var(y_train)
    var_e = var_p * (1 - h2)
    pev = jnp.diag(C22_inv) * var_e
    
    # Genetic variance can be estimated from the EBVs
    var_a = jnp.var(all_ebv)
    ...
    return PredictionResults(
        ...
        h2_used=h2,
        var_components={'var_a': var_a, 'var_e': var_e}
    )
